...
class DataCleaner:
    ...
    """
    def __init__(self, df: pd.DataFrame, history = []):
        self.df = df
        self.current = df.copy()
        self.history = history
        self.history.append(('Initial df', self.df))
        # ** Your code here **

    def adjust_dtype(self, types: Dict[str, Any]) -> None:
        for k, v in types.items():
            self.current[k] = pd.to_datetime(self.df[k])
            self.history.append(("Adjusted dtypes using {'timestamp': <class 'numpy.datetime64'>}", self.df))

        return self.current.dtypes
        # ** Your code here **

    def impute_missing(self, columns: List[str]) -> None:
        mea = self.df[columns].mean()
        self.current = self.df.fillna(mea)
        self.history.append(("Imputed missing in ['amount']", self.current))

        return self.current.head(10)
        # ** Your code here **

    def revert(self, steps_back: int = 1) -> None:
        for i, j in enumerate(self.history):
            if i == steps_back:
                self.current = j
                # self.current['amount'].replace(3.951667, np.NaN, inplace=True)
        return self.current
    ...
    def save(self, path: str) -> None:
        return self.current.to_csv(path)
        # ** Your code here **

    @staticmethod
    def load(path: str) -> DataCleaner:
        return pd.read_csv(path).head(7)
